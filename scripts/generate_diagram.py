...
def generate():
    ...
    if pipeline_dir.exists():
        for d in sorted(pipeline_dir.iterdir()):
            if d.is_dir() and re.match('p\\d+_', d.name):
    ...
    for stage in stages:
        ...
        lines.append('        end')
        if 'parse' in stage_id:
            lines.append(f"        {stage_id}_{stage['scripts'][-1]} -->|Write| Parsed[(parsed.jsonl)]")
        if 'dedup' in stage_id:
            lines.append(f"        Parsed -->|Read| {stage_id}_{stage['scripts'][0]}")
        if 'dedup' in stage_id:
            lines.append(f"        {stage_id}_{stage['scripts'][-1]} -->|Write| Clean[(clean.jsonl)]")
        if 'eda' in stage_id:
            lines.append(f"        Clean -->|Read| {stage_id}_{stage['scripts'][0]}")
        if 'eda' in stage_id:
            lines.append(f"        {stage_id}_{stage['scripts'][-1]} -->|Write| Summary[(eda_summary.json)]")
        if 'download' in stage_id:
            lines.append(f"        {stage_id}_{stage['scripts'][-1]} -->|Write| Raw[(data/raw/)]")
        if 'parse' in stage_id:
            lines.append(f"        Raw -->|Read| {stage_id}_{stage['scripts'][0]}")
    lines.append('    end')
    if tests:
