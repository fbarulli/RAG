...
# ---------------------------------------------------------------------------
# Document loading + stratified sampling
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        for i, doc in enumerate(docs, 1):
            ...
            output.append({
                ...
                "ner_category": doc.get("ner_category", "OTHER"),
                "ner_primary_entity": doc.get("ner_primary_entity"),
                "prompt_strategy": strategy_name,
                "variations": [v for v in variations[:3] if v],
            })
...
def _save(queries: list[dict], output_path: Path, metadata: dict) -> None:
    ...
    for r in queries:
        ...
        if key not in by_doc:
            by_doc[key] = {
                ...
                "section": r["section"],
                "topic": r.get("topic", -1),
                "ner_category": r.get("ner_category", "OTHER"),
                "ner_primary_entity": r.get("ner_primary_entity"),
                "prompt_results": {},
            }
    ...
    total_queries = sum(
        ...
        for v in doc["prompt_results"].values()
    )
...
async def main(
    ...
    logger.info(
        f"Generating queries: {len(sampled)} docs | "
        f"batch_size={batch_size} | "
        f"strategies={list(prompts.keys())} | "
        f"~{total_calls} LLM calls | "
        f"~{total_calls * (GAP_BETWEEN_CALLS + 5) / 60:.0f} min estimated"
    )
    ...
    metadata = {
        "description": "Stratified test queries for retrieval evaluation",
        "model": JUDGE_LLM,
    ...
    for batch_start in range(0, len(sampled) - batch_size + 1, batch_size):
        ...
        for strategy_name, strategy_info in prompts.items():
            # Skip already done
            skip = all(
                f"{doc['id']}::{strategy_name}" in done for doc in batch
            )
            if skip:
    ...
    logger.info(
        f"Done — {metadata['total_queries']} queries | "
        f"{metadata['total_documents']} docs | "
        f"{errors} errors | "
        f"{elapsed:.0f}s"
    )
