...
from production_pipeline.p05_evaluation.p01_generate_test_queries import (
    DEFAULT_CLEAN,
    DEFAULT_PROMPTS,
    DEFAULT_TOPIC_ASSIGNMENTS,
    GAP_BETWEEN_CALLS,
    JUDGE_LLM,
    _init_env,
    generate_batch,
    load_documents_with_topics,
    stratified_sample,
)
...
def sample_cross_course_clash(docs: list[dict], target_docs: int = 40) -> list[dict]:
    ...
    # Cache lowered text once to avoid repeated computation
    doc_texts = [(d, (d["question"] + " " + d["answer"]).lower()) for d in docs]

    # Build per-tool groups; a doc can belong to multiple tools
    ...
    # Deduplicate — docs can appear in multiple tool groups
    sampled = list({d["id"]: d for d in sampled}.values())
    logger.info(
        f"🎯 [Tier: cross_course_clash] Sampled {len(sampled)} unique docs "
        "targeting universal tool chains."
    )
    return sampled
...
def sample_intra_topic_battle(docs: list[dict], target_docs: int = 50) -> list[dict]:
    ...
    # Top-5 densest clusters only
    dense_topics = [
        t for t, _ in sorted(topic_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ]
    ...
    logger.info(
        f"⚔️ [Tier: intra_topic_battle] Sampled {len(sampled)} docs "
        f"strictly inside dense clusters: {dense_topics}"
    )
    return sampled
    ...
    try:
        with open(output_path, encoding="utf-8") as f:
            for line in f:
                ...
                row_id = row.get("id", "")
                # id format: query__{strategy}__{expected_id}_{idx}
                # Split on __ gives exactly ["query", strategy, "{expected_id}_{idx}"]
                # provided strategy and expected_id themselves contain no __
                parts = row_id.split("__")
                if len(parts) != 3:
                    continue

                _, strategy, tail = parts
                # Strip the trailing _{idx} suffix to recover expected_id
...
async def run_tier_generation(
    ...
    for batch_start in range(0, len(sampled_docs), batch_size):
        ...
        logger.info(
            f"🚀 [{tier_name.upper()}] Batch {bn}/{total_batches} "
            f"| Processing {len(batch)} source docs"
        )

        for strategy_name, strategy_info in prompts.items():
            needed = [
                d for d in batch
                if f"{d['id']}::{strategy_name}::{tier_name}" not in done_cache
            ]
            if not needed:
            ...
            with open(output_path, "a", encoding="utf-8") as f:
                for r in results:
                    ...
                    for idx, query_text in enumerate(r.get("variations", []), 1):
                        ...
                        row = {
                            # id format must stay stable — cache reconstruction depends on it
                            "id": f"query__{strategy_name}__{expected_id}_{idx}",
                            "question": query_text,
                            "answer": r["original_question"],
                            "course": r["course"],
                            "expected_id": expected_id,
                            "query_type": tier_name,
                            "section": r.get("section", ""),
                        }
                        f.write(json.dumps(row, ensure_ascii=False) + "\n")

            # Mark all just-written docs as done so later tiers don't re-run them
            for d in needed:
...
async def main() -> None:
    ...
    # Rebuild crash-resilient cache from existing output rows
    done_cache = _load_existing_cache(args.output)

    # Ensure output directory exists before any appends
    args.output.parent.mkdir(parents=True, exist_ok=True)

    # Deterministic shuffle for reproducible tier splits across restarts
    random.seed(42)
    random.shuffle(all_docs)

    # Tier 1 — Global Health: standard stratified sample
    global_docs = stratified_sample(all_docs, args.n_global)
    used_ids = {d["id"] for d in global_docs}

    # Tier 2 & 3 drawn from the remaining corpus only — mutual exclusivity guaranteed
    ...
    logger.info(
        f"📊 Tier split | Global: {len(global_docs)} "
        f"| Clash: {len(clash_docs)} "
        f"| Battle: {len(battle_docs)}"
    )
