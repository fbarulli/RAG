...
# Required files to consider a cache entry valid (more flexible)
...
class ONNXModelLoader:
    ...
    def __init__(
        ...
        # Generate a safe, unique hash for the cache directory
    ...
    def _is_cache_valid(self) -> bool:
        ...
        # Check for critical model artifacts
        for file in CACHE_REQUIRED_FILES:
            if not (path / file).exists():
                logger.debug("Cache missing file: %s", file)
                return False

        # Check for at least one tokenizer artifact (more flexible)
    ...
    def _load_from_cache(self) -> Tuple[Any, Any]:
        ...
        try:
            model = ORTModelForSequenceClassification.from_pretrained(
                cache_path_str, provider=self.provider
            )
    ...
    def _export_and_cache(self) -> Tuple[Any, Any]:
        ...
        try:
            ...
            model = ORTModelForSequenceClassification.from_pretrained(
                self.model_name, export=True, provider=self.provider
            )

            # 3. Save both to local cache
