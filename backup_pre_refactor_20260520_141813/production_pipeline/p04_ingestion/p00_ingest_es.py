...
Run: uv run python -m production_pipeline.p04_ingestion.p00_ingest_es --model "BAAI/bge-base-en-v1.5"
"""
import argparse
import json
import torch
...
ES_INDEX = "faqs"
BATCH_SIZE = 100
# Vector field config for hybrid search
VECTOR_FIELD = "question_vector"
VECTOR_DIM = 768  # Will be auto-detected from model
        ...
        "mappings": {
            "properties": {
                ...
                VECTOR_FIELD: {
                    ...
                    "dims": dims,
                    "index": True,
                    "similarity": "cosine"  # Match Qdrant distance
                }
            }
        }
    }
...
def main(model_name: str, input_path: Path = DEFAULT_INPUT, host: str = ES_HOST, index: str = ES_INDEX):
    ...
    for i, (doc, vec) in enumerate(zip(docs, vectors)):
        action = {
            "_index": index,
            "_id": doc.id,  # Use FAQDocument.id as ES _id for idempotency
            "_source": {
                "es_id": doc.id,
