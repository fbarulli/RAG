...
# Thread-safe cache: maps reranker_name -> {"config", "display_name", "model"}.
# Never invalidated at runtime — restart the process or call _clear_reranker_cache()
...
def _get_reranker_entry(reranker_name: str) -> Optional[dict]:
    ...
    # Slow path: double-checked lock to prevent redundant loads under concurrency
    with _CACHE_LOCK:
        if reranker_name in _LOADED_RERANKER_CACHE:
            return _LOADED_RERANKER_CACHE[reranker_name]
        ...
        if len(rerankers) > 1:
            logger.debug(
                f"Multiple rerankers found for '{reranker_name}', "
                f"using first: {rerankers[0].get('name')}"
            )

        config = rerankers[0]
        # Resolve display fields once at cache-store time rather than on every call
        entry = {
            "config": config,
            "display_name": config.get("short_name") or config.get("name"),
...
def evaluate_with_reranker(
    ...
    # Guard: no reranker name provided
    ...
    try:
        ...
        # Time only the actual reranking work, not cache lookup or setup
        start = time.perf_counter()
        reranked_ids, rerank_latency = run_reranking(
            reranker_config=entry["config"],
            query=query,
        ...
        metrics = {
            "reranker_name": entry["display_name"],
            "reranker_model": entry["model"],
        ...
        return (
            _safe_slice_ids(retrieved_candidates, top_k),
            {"reranker_name": "error", "reranker_latency_ms": 0.0},
        )