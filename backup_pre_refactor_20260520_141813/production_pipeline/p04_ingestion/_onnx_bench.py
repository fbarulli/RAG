...
def _execute_vector_search(client: QdrantClient, collection: str, query_vector: List[float], config: Dict[str, Any]) -> Optional[Any]:
    ...
    return run_entity_boosted_retrieval(
        client=client, collection=collection, query_vector=query_vector,
        course_filter=config.get("course_filter", "all"), config=config,
        top_k=40, ner_category=None, ner_primary_entity=None,
    )
...
# ==============================================================================
# Scoring and Iteration Loop Slices
# ==============================================================================
...
def run_benchmark_loop(
    ...
    encoder: Any, 
    embedding_model: SentenceTransformer,
    topic_map: Optional[Dict[str, Any]] = None  # ADDED: Topic map passing
) -> Optional[Any]:
    ...
    for idx, query_item in enumerate(test_set, start=1):
        try:
            evaluated_item = _evaluate_single_query(
                client, query_item, idx, collection, config, encoder, embedding_model, topic_map
            )
            if evaluated_item:
    ...
    logger.info("🏆 Final Performance Matrix for %s: Hit@5=%s MRR=%s", 
                model_name, f"{summary.hit_rate_5:.1%}", f"{summary.mrr:.4f}")
    return summary


# ==============================================================================
# Helper Context Slices for Bootstrapping
# ==============================================================================
...
def execute_matrix_evaluation(
    ...
    embedding_model: SentenceTransformer,
    topic_map: Optional[Dict[str, Any]] = None,  # ADDED: Topic map boundary
    ...
    for reranker_entry in reranker_entries:
        ...
        try:
            ...
            encoder = compile_onnx_runtime_node(
                model_key=reranker_entry["model"],
                max_length=reranker_entry.get("max_length", 512),
                provider="CPUExecutionProvider"
            )
            
            # Pass topic map metadata cleanly into loop scopes
