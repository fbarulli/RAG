...
def audit_code_blocks(docs: list[FAQDocument]) -> dict:
    ...
    results = {
        ...
        'truncated_blocks': 0,
        'malformed_blocks': 0,
        'by_course': defaultdict(lambda: {'total': 0, 'complete': 0, 'truncated': 0})
    }
    
    # Patterns for complete vs truncated code blocks
    complete_pattern = re.compile(r'```(?:\w+)?\n(?:[^\n]*\n)+```', re.MULTILINE)
    truncated_pattern = re.compile(r'```(?:\w+)?\n[^\n]*(?:\n[^\n]*){0,2}(?!\n```)', re.MULTILINE)
    
    for doc in docs:
        blocks = re.findall(r'```.*?```', doc.answer, re.DOTALL)
        if not blocks:
            continue
...
def audit_content_quality(docs: list[FAQDocument]) -> dict:
    ...
    results = {
        ...
        'answer_length_outliers': [],
        'question_answer_ratio_outliers': []
    }
    ...
    for doc in docs:
        ...
        # Check whitespace
        if len(doc.answer) - len(re.sub(r'\s', '', doc.answer)) > len(doc.answer) * 0.5:
            results['excessive_whitespace'] += 1
        
        # Track unusual characters
        for char in doc.answer:
            if unicodedata.category(char).startswith('C'):  # Control chars
                results['unusual_chars'][repr(char)] += 1
        
        # Flag length outliers (using simple IQR-like heuristic)
        if len(doc.answer) < 10 or len(doc.answer) > 2000:
            results['answer_length_outliers'].append({
                'id': doc.id, 'course': doc.course, 'length': len(doc.answer),
                'question': doc.question[:50], 'answer_snippet': doc.answer[:100]
            })
        
        # Flag weird Q/A ratios (very short question, very long answer or vice versa)
        q_len, a_len = len(doc.question), len(doc.answer)
        if (q_len < 20 and a_len > 1000) or (q_len > 200 and a_len < 50):
            results['question_answer_ratio_outliers'].append({
                'id': doc.id, 'q_len': q_len, 'a_len': a_len,
                'question': doc.question[:50], 'answer_snippet': doc.answer[:100]
            })
...
def audit_code_patterns(docs: list[FAQDocument]) -> dict:
    ...
    for doc in docs:
        # Fenced code blocks
        fenced = re.findall(r'```(\w+)?', doc.answer)
        for lang in fenced:
            lang_counts[lang or 'unknown'] += 1
        
        # Inline code (single backticks)
        inline = re.findall(r'`([^`]+)`', doc.answer)
        for code in inline:
    ...
    return {
        'fenced_languages': dict(lang_counts.most_common(10)),
        'inline_code_types': dict(inline_code_counts.most_common(10))
    }
...
def run_audit(input_path: Path = None, sample_size: int = None) -> dict:
    ...
    return {
        'metadata': audit_metadata(docs),
        'content_quality': audit_content_quality(docs),
        'code_blocks': audit_code_blocks(docs),
        'code_patterns': audit_code_patterns(docs),
        'summary': {
            'total_docs': len(docs),
            'docs_with_code': audit_code_blocks(docs)['total_with_code'],
            'code_preservation_rate': round(
                audit_code_blocks(docs)['complete_blocks'] / 
                max(1, audit_code_blocks(docs)['total_with_code']) * 100, 1
            ) if audit_code_blocks(docs)['total_with_code'] > 0 else 0
        }
    }
