...
TEST_MODELS = [
    "BAAI/bge-small-en-v1.5",          # baseline: 384-dim, fast
    "BAAI/bge-base-en-v1.5",           # recommended: 768-dim, balanced
    "sentence-transformers/all-mpnet-base-v2",  # strong generalist
    "nomic-ai/nomic-embed-text-v1.5",  # long-context, newer
    "intfloat/e5-small-v2",            # query-doc optimized
]
...
def compute_cluster_quality(embeddings: np.ndarray, topics: list[int]) -> dict:
    ...
    # Use cosine metric for silhouette since SentenceTransformers are optimized for cosine similarity
...
def run_clustering(
    ...
    """Run BERTopic and return comprehensive metrics."""
    # Start tracking time and peak memory
    start_time = time.time()
    tracemalloc.start()
    
    # Load or compute embeddings AND return the loaded embedder object
    embeddings, embedder = load_or_compute_embeddings(
        questions, model_name, cache_dir, use_cache
    )
    
    # Configure HDBSCAN (BERTopic uses UMAP which outputs euclidean spaces by default)
    ...
    topic_model = BERTopic(
        embedding_model=embedder,  # Pass instantiated object to prevent redundant loading
        hdbscan_model=hdbscan_model,
        ctfidf_model=ClassTfidfTransformer(reduce_frequent_words=True),  # Correctly placed!
        verbose=False,
    )
    ...
    return {
        ...
        "avg_keyword_overlap": round(keyword_overlap, 4) if keyword_overlap else None,
        "mean_confidence": round(mean_conf, 4),
        "low_confidence_count": low_conf_count,
        "low_confidence_ratio": round(low_conf_count / len(valid_probs), 4) if valid_probs else None,
        "topic_size_stats": size_stats,
        "silhouette_score": quality["silhouette_score"],
        "davies_bouldin_index": quality["davies_bouldin_index"],
...
def print_comparison_table(results: list[dict]) -> None:
    ...
    for r in sorted(results, key=lambda x: x["outlier_ratio"]):
        ...
        print(f"{model_short:<45} "
              f"{r['outlier_ratio']:>9.1%} "
              f"{r['num_topics']:>8} "
              f"{r['silhouette_score'] or 'N/A':>12} "
              f"{r['runtime_seconds']:>9.1f}s "
              f"{r['peak_memory_mb']:>9.0f}MB")
    
    print("=" * 100)
    
    # Highlight best performers
    best_outliers = min(results, key=lambda x: x["outlier_ratio"])
    best_silhouette = max((r for r in results if r["silhouette_score"]), 
...
def main() -> None:
    ...
    # Print comparison table if multiple models tested
