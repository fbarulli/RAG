...
def find_duplicates_in_group(
    ...
    threshold: float,
    course_aware: bool = True
) -> tuple[list[FAQDocument], list[dict]]:
    """
    ...
    # If course-aware, subgroup by course first
    if course_aware:
    ...
    for group in groups:
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                if sim >= threshold:
                    # Keep the one with shorter ID (deterministic)
                    keep_idx, remove_idx = (i, j) if group[i].id < group[j].id else (j, i)
                    removed_ids.add(remove_idx)
                    
                    removed_info.append({
                        "kept_id": group[keep_idx].id,
                        "removed_id": group[remove_idx].id,
                        "question": group[remove_idx].question,
                        "similarity": f"{sim:.4f}",
                        "course": group[remove_idx].course,
                        "section": group[remove_idx].section or "",
                        "kept_answer": group[keep_idx].answer,
                        "removed_answer": group[remove_idx].answer,
                    })
        
        # Add kept docs from this group
...
def main(
    ...
    course_aware: bool = DEFAULT_COURSE_AWARE
):
    # Load documents
    docs = []
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Similarity threshold for dedup (default: {DEFAULT_THRESHOLD})"
    )
    parser.add_argument(
        "--global-dedup", action="store_true",
        help="Deduplicate across all courses (default: course-aware only)"
    )
    return parser
...
if __name__ == "__main__":
    ...
    main(
        ...
        course_aware=not args.global_dedup
    )