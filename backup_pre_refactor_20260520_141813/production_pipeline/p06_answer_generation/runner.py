...
class QueryRetriever:
    ...
    def retrieve(self, query: str, top_k: int = 5, course: Optional[str] = None, mode: str = "vector") -> list[str]:
        ...
        from production_pipeline.p04_ingestion._benchmark_metrics.retrievers import (
        ...
        if mode == "entity_boosted":
            ...
            result = run_hybrid_rrf_retrieval(
                client=self.client,
                collection=self.collection,
                query_vector=vector,
                es=None,                    # TODO: pass ES client later
                es_index="faqs",
                query_text=query,
                course_filter=course,
                config=config,
