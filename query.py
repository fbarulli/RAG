...
try:
    from rag_pipeline.ingestion._benchmark_metrics.retrievers import run_entity_boosted_retrieval
    from rag_pipeline.ingestion._benchmark_reranker import evaluate_with_reranker
    from rag_pipeline.ingestion._reranker_runner import run_reranking
...
try:
    result = run_entity_boosted_retrieval(client=client, collection='faqs_bge_base_en_v1_5', query_vector=dummy_vector, course_filter='machine-learning-zoomcamp', config={'boost_question': 5.0, 'boost_text': 5.0, 'rrf_k': 60}, top_k=40, ner_category=None, ner_primary_entity=None)
...
try:
    ...
    reranked, latency = run_reranking(reranker_config={'model': 'BAAI/bge-reranker-base', 'name': 'bge-reranker-base'}, query='test query', candidates=candidates, top_k=5)
...
try:
    ...
    pairs = [('What is the capital of France?', 'Paris is the capital of France.'), ('What is machine learning?', 'Machine learning is a field of AI.')]
    t0 = time.perf_counter()
