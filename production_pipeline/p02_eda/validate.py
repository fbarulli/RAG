...
def validate_dataset(docs: list[Union[dict, FAQDocument]], 
                     valid_courses: set[str] | None = None,
                     min_answer_len: int = 10) -> list[str]:
    """Run sanity checks on FAQ dataset. Returns list of issue strings."""
    issues = []
    if valid_courses is None:
        valid_courses = {'llm-zoomcamp', 'mlops-zoomcamp', 'de-zoomcamp', 
                         'ml-zoomcamp', 'data-engineering-zoomcamp', 'machine-learning-zoomcamp'}
    ...
    artifacts = sum(1 for d in docs 
                    if '<{' in (d.answer if isinstance(d, FAQDocument) else d['answer']) 
                    or '<IMAGE' in (d.answer if isinstance(d, FAQDocument) else d['answer']))
    if artifacts:
