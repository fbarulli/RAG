...
from ._tfidf_stopwords import (
    BRIDGE_CONCEPT_WHITELIST,
    apply_code_stripping,
    balance_corpus,
    build_stopword_list,
    load_stopwords,
    save_stopwords,
)
# In your _tfidf_course_analysis.py, right after importing Paths:


# Debug Paths resolution
...
# ── swap these two lines for your project's Paths + get_logger if available ──
logging.basicConfig(
    level=logging.INFO,
...
# TF-IDF hyperparameters — single source of truth, flows into _meta output
...
def load_cleaned_data(eda_stats_path: Path | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    if eda_stats_path is None:
        eda_stats_path = Path("/workspaces/LLM/production_pipeline/experiments/eda_summary.json")
    ...
    try:
        ...
        if len(df) < before:
            logger.warning(
                f"[load_cleaned_data] Dropped {before - len(df)} empty-text rows"
            )

        if df["course"].isna().any():
            raise ValueError(
                f"[load_cleaned_data] {df['course'].isna().sum()} rows have null course."
            )

        logger.info(
            f"[load_cleaned_data] {len(df)} documents from {path.name} | "
            f"courses: {sorted(df['course'].unique())} | "
            f"counts: {df['course'].value_counts().to_dict()}"
        )
        ...
        if imbalance_ratio > 3.0:
            logger.warning(
                f"[load_cleaned_data] Course imbalance detected: "
                f"max/min ratio = {imbalance_ratio:.2f}. "
                f"Consider balance_corpus() before TF-IDF fitting."
            )
        ...
        if eda_stats_path is not None:
            ...
            if not eda_path.exists():
                logger.warning(
                    f"[load_cleaned_data] EDA stats path not found: {eda_path}. "
                    f"Proceeding without EDA-derived stopwords."
                )
            else:
...
def prepare_corpus(
    df: pd.DataFrame,
    balance_strategy: str = "oversample_min",
    strip_code: bool = True,
    exclude_homework: bool = True,
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        # Step 1 — homework exclusion
        if exclude_homework and "section" in df.columns:
            mask = df["section"].str.contains("homework", case=False, na=False)
            n_hw = mask.sum()
            df   = df[~mask].copy()
            logger.info(
                f"[prepare_corpus] Excluded {n_hw} homework chunks "
                f"({n_hw / original_len:.1%} of corpus)"
            )
        elif exclude_homework and "section" not in df.columns:
            logger.warning(
                "[prepare_corpus] exclude_homework=True but 'section' column "
                "not found — skipping homework exclusion."
            )

        # Step 2 — code stripping
        if strip_code:
        ...
        logger.info(
            f"[prepare_corpus] {original_len} -> {len(df)} chunks after preparation"
        )
        return df
...
) -> tuple[dict, TfidfVectorizer, pd.DataFrame, np.ndarray]:
    """
    ...
    try:
        if text_col not in df.columns:
            raise ValueError(
                ...
                f"Did you run prepare_corpus() first? "
                f"Available columns: {list(df.columns)}"
            )
        if df["course"].nunique() < 2:
            raise ValueError(
                f"Need at least 2 courses for meaningful TF-IDF comparison. "
                f"Found: {df['course'].unique()}"
            )

        course_texts = (
            df.groupby("course")[text_col]
            .agg(" ".join)
            .reset_index()
            .rename(columns={text_col: "agg_text"})
        )
        chunk_counts  = df.groupby("course").size().rename("chunk_count")
        course_texts  = course_texts.join(chunk_counts, on="course")

        # Validate no course produced an empty aggregation
        empty_courses = course_texts[course_texts["agg_text"].str.strip() == ""]["course"].tolist()
        if empty_courses:
            raise ValueError(
                f"Courses produced empty aggregated text after preparation: {empty_courses}. "
                f"Check strip_code and exclude_homework settings."
            )
        ...
        tfidf_dense   = tfidf_matrix.toarray()        # single dense conversion
        feature_names = vectorizer.get_feature_names_out()

        if len(feature_names) == 0:
            raise ValueError(
                "Vocabulary is empty after fitting — stopword list may be too aggressive. "
                "Check VECTORIZER_PARAMS min_df and the stopword list."
            )
        ...
        logger.info(
            f"[fit_tfidf] Fitted on {len(course_texts)} courses | "
...
def run_iterative_tfidf(
    df: pd.DataFrame,
    eda_stats: dict,
    top_n: int = 50,
    iterations: int = 2,
    stopwords_output_dir: Path | None = None,
) -> tuple[dict, TfidfVectorizer, pd.DataFrame, np.ndarray, list[str]]:
    """
    ...
    try:
        ...
        results, vectorizer, course_texts, tfidf_dense = fit_tfidf(
            df, stopwords, top_n
        )
        ...
        for i in range(1, iterations):
            ...
            if stopwords_output_dir:
                save_stopwords(
                    ...
                    f"stopwords_pass{i + 1}.txt",
                )

        logger.info(
            f"[run_iterative_tfidf] Complete — "
            f"{iterations} pass(es), final stopword count: {len(stopwords)}"
        )
...
def find_bridge_concepts(
    ...
    try:
        ...
        if min_courses < 2:
            raise ValueError(
                f"min_courses must be >= 2 for a bridge to exist, got {min_courses}"
            )
        ...
        for course, terms in tfidf_results.items():
            if not terms:
                logger.warning(
                    f"[find_bridge_concepts] Course '{course}' has empty term list."
                )
                continue
        ...
        bridges = {
            term: course_scores
            for term, course_scores in term_course_scores.items()
            if len(course_scores) >= min_courses
        }
        ...
        logger.info(
            f"[find_bridge_concepts] "
            f"top_n={top_n}, min_courses={min_courses} "
            f"-> {len(bridges)} bridge concepts"
        )
        return bridges
...
def characterise_bridge_concepts(
    df: pd.DataFrame,
    ...
    try:
        ...
        if not bridges:
            logger.warning(
                "[characterise_bridge_concepts] bridges dict is empty — "
                "returning empty characterisations."
            )
            return {}
        ...
        for term in list(bridges.keys())[:max_bridges]:
            ...
            for course in bridges[term]:
                ...
                if not course_chunks:
                    logger.warning(
                        f"[characterise_bridge_concepts] "
                        f"No chunks found for course '{course}' and term '{term}'."
                    )
                    continue
                ...
                if not matching:
                    logger.debug(
                        f"[characterise_bridge_concepts] "
                        f"Term '{term}' not found in any '{course}' chunk."
                    )
                    continue
        ...
        logger.info(
            f"[characterise_bridge_concepts] "
            f"Characterised {len(characterisations)} bridge concepts "
            f"(capped at {max_bridges})"
        )
        return characterisations

    except Exception:
        logger.error(
            "[characterise_bridge_concepts] Failed.\n" + traceback.format_exc()
        )
        raise
...
def enrich_chunks(
    ...
    try:
        ...
        if missing_courses:
            logger.warning(
                f"[enrich_chunks] Courses in df missing from tfidf_results: "
                f"{missing_courses}. Their fingerprints will be empty."
            )

        course_fingerprints = {
            course: [t for t, _ in terms[:top_n]]
            for course, terms in tfidf_results.items()
        }
        ...
        def _enrich_row(row: pd.Series) -> pd.Series:
            try:
                ...
                if chunk_bridges:
                    note = (
                        f"This chunk discusses {', '.join(chunk_bridges[:3])}"
                        f" from a {row['course']} perspective."
                    )
                else:
                ...
                logger.error(
                    f"[enrich_chunks._enrich_row] Failed on row index {row.name}.\n"
                    + traceback.format_exc()
                )
        ...
        logger.info(
            f"[enrich_chunks] {len(df)} chunks enriched — "
            f"{n_bridged} ({n_bridged / len(df):.1%}) touch >= 1 bridge concept"
        )
        return df
...
def compute_similarity_matrix(
    ...
    try:
        if tfidf_dense.shape[0] != len(course_names):
            raise ValueError(
                f"tfidf_dense has {tfidf_dense.shape[0]} rows but "
                f"{len(course_names)} course names were provided."
            )
...
def pairwise_cooccurrence_score(
    df: pd.DataFrame,
    tfidf_results: dict,
    top_n: int = 10,
    text_col: str = "clean_text",
    ...
    try:
        ...
        for course, terms in tfidf_results.items():
            ...
            if not chunks:
                logger.warning(
                    f"[pairwise_cooccurrence_score] No chunks for course '{course}'."
                )
                scores[course] = 0.0
                continue
            if len(top_terms) < 2:
                logger.warning(
                    f"[pairwise_cooccurrence_score] "
                    f"Course '{course}' has fewer than 2 terms — skipping pairs."
                )
                scores[course] = 0.0
                continue
        ...
        logger.error(
            "[pairwise_cooccurrence_score] Failed.\n" + traceback.format_exc()
        )
        raise
    ...
    try:
        ...
        for trial in range(n_trials):
            ...
            sim_df = compute_similarity_matrix(
                tfidf_dense, course_texts["course"].tolist()
            )
            trial_scores.append({
                "mean_pairwise_sim": mean_pairwise_similarity(sim_df),
                "mean_uniqueness":   float(np.mean(list(uniqueness_ratio(results, top_n).values()))),
                "mean_coherence":    float(np.mean(list(pairwise_cooccurrence_score(df_s, results, top_n).values()))),
            })

        agg = pd.DataFrame(trial_scores)
        result = {
            "mean": agg.mean().round(4).to_dict(),
            "std":  agg.std().round(4).to_dict(),
        }
...
def count_vectorizer_baseline(
    ...
    try:
        ...
        return {
            "mean_pairwise_sim": round(mean_pairwise_similarity(sim_df), 4),
            "mean_uniqueness":   round(float(np.mean(list(uniqueness_ratio(results, top_n).values()))), 4),
            "mean_coherence":    round(float(np.mean(list(pairwise_cooccurrence_score(df, results, top_n).values()))), 4),
        }
...
def score_config(
    ...
    try:
        ...
        for c in course_texts["course"]:
            ...
            per_course[c] = {
                "uniqueness":  uniqueness.get(c, 0.0),
                "coherence":   coherence.get(c, 0.0),
                "chunk_count": chunk_count,
            }

        return {
            "mean_pairwise_sim": round(mean_pairwise_similarity(sim_df), 4),
            "mean_uniqueness":   round(float(np.mean(list(uniqueness.values()))), 4),
            "mean_coherence":    round(float(np.mean(list(coherence.values()))), 4),
...
# ─────────────────────────────────────────────────────────────────────────────
# 7. MULTI-PERSPECTIVE RETRIEVAL
# ─────────────────────────────────────────────────────────────────────────────
def _tfidf_score_chunks(
    query: str,
    documents: list[dict],
    ...
    try:
        ...
        if "clean_text" not in documents[0]:
            raise KeyError(
                f"Document dicts must have 'clean_text'. "
                f"Found keys: {list(documents[0].keys())}"
            )
        ...
        if query_vector.nnz == 0:
            logger.warning(
                "[_tfidf_score_chunks] Query produced a zero vector — "
                "all tokens may be stopwords or OOV. Returning documents unranked."
            )
...
def multi_perspective_retrieve(
    query: str,
    documents: list[dict],
    vectorizer: TfidfVectorizer,
    bridges: dict[str, dict[str, float]],
...
def build_report(
    ...
    try:
        ...
        pairs = [
            ...
            for b in list(sim_df.index)[i + 1:]
        ]
        ...
        header = (
            f"  {'Method':<30} {'mean_pairwise_sim':>18} "
            f"{'mean_uniqueness':>16} {'mean_coherence':>15}"
        )
        lines += [header, "  " + sep2]
        ...
        def _row(name: str, d: dict) -> str:
            return (
                f"  {name:<30} "
                f"{d.get('mean_pairwise_sim', 0):>18.4f} "
                f"{d.get('mean_uniqueness', 0):>16.4f} "
                f"{d.get('mean_coherence', 0):>15.4f}"
            )
        ...
        for course, detail in scores.get("per_course", {}).items():
            lines.append(
                f"    {course:<35}  "
                f"uniqueness={detail['uniqueness']:.3f}  "
                f"coherence={detail['coherence']:.4f}  "
...
def save_results(
    ...
    try:
        ...
        if missing_cols:
            raise ValueError(
                f"enriched_df is missing columns from enrich_chunks(): {missing_cols}. "
                f"Ensure enrich_chunks() ran successfully before save_results()."
            )
        ...
        payload = {
            "_meta": {
                ...
                "courses":          list(tfidf_results.keys()),
                "stopword_count":   len(stopwords_used),
            },
            "top_terms_per_course":     tfidf_results,
            "bridge_concepts":          bridges,
            "bridge_characterisations": bridge_characterisations,
            "scores":                   scores,
            "baselines":                baselines,
        }
        ...
        # long-form terms CSV
        rows = [
            {"course": course, "rank": rank + 1, "term": term, "score": score}
            for course, terms in tfidf_results.items()
            for rank, (term, score) in enumerate(terms)
        ]
        pd.DataFrame(rows).to_csv(output_dir / "tfidf_terms_long.csv", index=False)

        # bridge concepts CSV
        bridge_rows = [
            {"term": term, "course": course, "tfidf_score": score}
            for term, course_scores in bridges.items()
        ...
        # enriched chunks — convert list columns to JSON strings for JSONL
        ...
        df_out.to_json(
            output_dir / "enriched_chunks.jsonl",
            orient="records",
            lines=True,
            force_ascii=False,
        )
...
def main() -> None:
    ...
    df = prepare_corpus(
        df,
        balance_strategy="oversample_min",
        strip_code=True,
        exclude_homework=True,
    )
    ...
    tfidf_results, vectorizer, course_texts, tfidf_dense, stopwords_used = (
        run_iterative_tfidf(
            df, eda_stats, top_n=50, iterations=2,
            stopwords_output_dir=stopwords_dir,
        )
    )
    ...
    baselines = {
        "null":             null_baseline(df, eda_stats, top_n=25, n_trials=10),
        "count_vectorizer": count_vectorizer_baseline(df, eda_stats, top_n=25),
    }

    # ── report
    report = build_report(
        tfidf_results, bridges, bridge_characterisations,
        sim_df, scores, baselines, course_texts, top_n=25,
    )
    print(report)
    ...
    save_results(
        tfidf_results, bridges, bridge_characterisations,
        sim_df, scores, baselines, enriched_df,
        report, stopwords_used, output_dir,
    )
    ...
    results, used_bridge = multi_perspective_retrieve(
        test_query, chunks, vectorizer, bridges, k_total=5
    )
    print(f"\n{'=' * 60}")
    ...
    for i, chunk in enumerate(results, 1):
        print(
            f"\n  [{i}] course={chunk.get('course')} | "
            f"bridges={chunk.get('bridge_concepts', [])} | "
            f"note={chunk.get('concept_note', '')}\n"
            f"      {chunk['clean_text'][:150]}..."
        )
