...
# Default thresholds for pass/warn/fail evaluation (fixed mode)
DEFAULT_THRESHOLDS = {
...
# Weights are normalised at runtime so they don't have to sum to 1.0 here.
_RAW_METRIC_WEIGHTS: dict[str, float] = {
    "outlier_ratio": 0.35,
    "avg_keyword_overlap": 0.25,
    "confidence_mean": 0.25,
    "cross_course_topics": 0.15,
}
_weight_total = sum(_RAW_METRIC_WEIGHTS.values())
METRIC_WEIGHTS: dict[str, float] = {
    k: v / _weight_total for k, v in _RAW_METRIC_WEIGHTS.items()
}
...
def normalize_metric(
    ...
    if max_val == min_val:
        return 1.0  # All models identical on this metric → no discrimination
...
def compute_composite_score(
    ...
    scores: dict[str, float] = {
        "outlier_ratio": normalize_metric(
            result["outlier_ratio"], outlier_vals, lower_is_better=True
        ) if result["outlier_ratio"] is not None else 0.5,

        "avg_keyword_overlap": normalize_metric(
            result["avg_keyword_overlap"], overlap_vals, lower_is_better=True
        ) if result["avg_keyword_overlap"] is not None else 0.5,

        "confidence_mean": normalize_metric(
            result["confidence_mean"], conf_vals, lower_is_better=False
        ) if result["confidence_mean"] is not None else 0.5,

        "cross_course_topics": normalize_metric(
            result["cross_course_topics"], cross_vals, lower_is_better=True
        ) if result["cross_course_topics"] is not None else 0.5,
    }
...
def evaluate_status_fixed(
    ...
    if metrics["outlier_ratio"] >= thresholds["outlier_ratio_max"]:
        violations.append(
            f"outlier_ratio {metrics['outlier_ratio']:.1%} >= {thresholds['outlier_ratio_max']:.1%}"
        )

    if metrics["avg_keyword_overlap"] is not None:
        if metrics["avg_keyword_overlap"] >= thresholds["keyword_overlap_max"]:
            violations.append(
                f"keyword_overlap {metrics['avg_keyword_overlap']:.4f} >= {thresholds['keyword_overlap_max']:.4f}"
            )

    if metrics["confidence_mean"] is not None:
        if metrics["confidence_mean"] < thresholds["confidence_min"]:
            violations.append(
                f"confidence_mean {metrics['confidence_mean']:.3f} < {thresholds['confidence_min']:.3f}"
            )

    if metrics["cross_course_topics"] is not None:
        if metrics["cross_course_topics"] > thresholds["cross_course_max"]:
            violations.append(
                f"cross_course_topics {metrics['cross_course_topics']} > {thresholds['cross_course_max']}"
            )
...
def _adaptive_thresholds(
    ...
    upper_pct = (1.0 - percentile) * 100  # e.g. 75 for percentile=0.25
    lower_pct = percentile * 100           # e.g. 25 for percentile=0.25

    return {
        "outlier_thresh": (
            float(np.percentile(outlier_vals, upper_pct))
            if outlier_vals else DEFAULT_THRESHOLDS["outlier_ratio_max"]
        ),
        "overlap_thresh": (
            float(np.percentile(overlap_vals, upper_pct))
            if overlap_vals else DEFAULT_THRESHOLDS["keyword_overlap_max"]
        ),
        "conf_thresh": (
            float(np.percentile(conf_vals, lower_pct))
            if conf_vals else DEFAULT_THRESHOLDS["confidence_min"]
        ),
    }


def evaluate_status_adaptive(
    metrics: dict[str, Any],
    all_results: list[dict[str, Any]],
    ...
    pct_label = f"p{(1 - percentile) * 100:.0f}"  # label shown in messages

    if metrics["outlier_ratio"] is not None and metrics["outlier_ratio"] > thresh["outlier_thresh"]:
        violations.append(
            f"outlier_ratio {metrics['outlier_ratio']:.1%} > {thresh['outlier_thresh']:.1%} ({pct_label})"
        )

    if metrics["avg_keyword_overlap"] is not None and metrics["avg_keyword_overlap"] > thresh["overlap_thresh"]:
        violations.append(
            f"keyword_overlap {metrics['avg_keyword_overlap']:.4f} > {thresh['overlap_thresh']:.4f} ({pct_label})"
        )

    if metrics["confidence_mean"] is not None and metrics["confidence_mean"] < thresh["conf_thresh"]:
        violations.append(
            f"confidence_mean {metrics['confidence_mean']:.3f} < {thresh['conf_thresh']:.3f} ({pct_label})"
        )

    # Cross-course topics keeps a hard fixed cap regardless of mode.
    if metrics["cross_course_topics"] is not None:
        if metrics["cross_course_topics"] > DEFAULT_THRESHOLDS["cross_course_max"]:
            violations.append(
                f"cross_course_topics {metrics['cross_course_topics']} > {DEFAULT_THRESHOLDS['cross_course_max']}"
            )
...
def find_pareto_front(results: list[dict[str, Any]]) -> list[str]:
    ...
    valid = [
        r for r in results
        if all(r.get(m) is not None for m in all_tracked)
    ]
...
def print_comparison_table(
    ...
    print(
        f"{'Model':<45} "
        f"{'Out%':>7} {'Overlap':>10} {'Conf':>8} {'Std':>8} {'Cross':>7} "
        f"{'Topics':>7} {'Score':>7} {'Status':>8} {'Violations':<40}"
    )
    print("-" * 160)
    ...
    for r in results:
        ...
        print(
            f"{r['model']:<45} "
            f"{r['outlier_ratio']:>6.1%} "
            f"{_format_optional(r['avg_keyword_overlap'], '.4f'):>10} "
            f"{_format_optional(r['confidence_mean'], '.3f'):>8} "
            f"{_format_optional(r['topic_size_std'], '.1f'):>8} "
            f"{str(r['cross_course_topics']) if r['cross_course_topics'] is not None else 'N/A':>7} "
            f"{r['num_topics']:>7} "
            f"{score_str:>7} "
            f"{r.get('status', '?'):>8} "
            f"{violations_str:<40}"
    ...
    if adaptive:
        ...
        print(
            f"Adaptive thresholds (top {percentile * 100:.0f}% pass): "
            f"outlier≤{thresh['outlier_thresh']:.1%} (p{pct:.0f}), "
            f"overlap≤{thresh['overlap_thresh']:.4f} (p{pct:.0f}), "
            f"conf≥{thresh['conf_thresh']:.3f} (p{percentile * 100:.0f})"
        )
    else:
        print(
            f"Fixed thresholds: outlier≤{thresholds['outlier_ratio_max']:.0%}, "
    ...
    scored = sorted(
        [r for r in results if "composite_score" in r],
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compare topic modeling results across embedding models"
    )
    parser.add_argument(
    ...
    # Adaptive threshold arguments
    parser.add_argument(
        "--adaptive", action="store_true",
        help="Use adaptive percentile-based thresholds instead of fixed values",
    )
    parser.add_argument(
        "--adaptive-method", type=str, choices=["percentile", "pareto"],
        default="percentile",
        help=(
            "Method for adaptive evaluation. "
            "'percentile' uses distribution-based cutoffs; "
            "'pareto' skips per-metric thresholds and reports the Pareto front."
        ),
    )
    parser.add_argument(
        "--percentile", type=float, default=0.25,
        help="Top-N fraction that should pass (0.25 = top 25%% pass). Only used with --adaptive.",
    )
    parser.add_argument(
        "--no-composite", action="store_true",
        help="Disable composite score calculation",
    )
    return parser
...
def _evaluate_all(
    ...
    for r in results:
        if adaptive and adaptive_method == "percentile":
            ...
            # Both fixed mode and pareto mode use fixed thresholds for per-model status.
...
def main() -> None:
    ...
    # Per-model pass/warn/fail
    _evaluate_all(results, thresholds, args.adaptive, args.adaptive_method, args.percentile)

    # Sort: composite score if available, else outlier_ratio then overlap
    if not args.no_composite:
        results.sort(key=lambda x: x.get("composite_score", 0.0), reverse=True)
    else:
        results.sort(key=lambda x: (
            x["outlier_ratio"],
            x["avg_keyword_overlap"] if x["avg_keyword_overlap"] is not None else 1.0,
        ))

    # Pareto front (always compute; used in reporting regardless of mode)
    ...
    summary: dict[str, Any] = {
        ...
        "adaptive_method": args.adaptive_method if args.adaptive else None,
        "percentile": args.percentile if args.adaptive else None,
        "composite_score_enabled": not args.no_composite,
        "results": results,
        "best_model": results[0]["model"] if results else None,
        "pareto_optimal": pareto_models or None,
        "metadata": {
            "total_models": len(results),
            "models_ok": sum(1 for r in results if r.get("status") == "OK"),
            "models_warn": sum(1 for r in results if r.get("status") == "WARN"),
            "models_fail": sum(1 for r in results if r.get("status") == "FAIL"),
        },
    }
    ...
    if results:
        ...
        # Pareto alternatives when the top composite-score model isn't Pareto-optimal
