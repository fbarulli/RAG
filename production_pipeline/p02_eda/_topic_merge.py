...
# ---------------------------------------------------------------------------
# Reclassification signals — extend as diagnostics reveal new patterns
# ---------------------------------------------------------------------------
...
_ADMIN_SIGNALS = {
    ...
    "lecture", "video", "live", "recorded", "session", "form",
    "confirmation", "email", "registration", "enroll", "books",
...
def merge(exp_dir: Path = EXP_DIR, output_path: Path = OUTPUT_PATH) -> None:
    ...
    files = sorted(f for f in exp_dir.glob("topic_assignments_*.json")
                   if f.name != "topic_assignments_all.json"
                   and "_validated" not in f.name)
    ...
    for f in files:
        ...
        logger.info(
            f"[merge] {model}: {len(data['assignments'])} assignments, "
            f"{reclassified} OTHER reclassified"
        )
