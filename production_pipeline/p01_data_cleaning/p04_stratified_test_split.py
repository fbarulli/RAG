...
Run:    uv run python -m production_pipeline.p01_data_cleaning.p04_stratified_test_split
...
def stratified_sample_imbalanced(
    docs: list[dict],
    n: int,
    stratify_by: list[str],
    seed: int,
    min_per_group: int = 3,      # NEW: Ensure small groups are represented
    max_per_group: Optional[int] = None,  # NEW: Cap large groups to prevent dominance
    exclude_outliers: bool = False,  # NEW: Optionally exclude topic -1 docs
) -> list[dict]:
    """
    ...
    # Calculate proportional allocation with bounds
    total = len(docs)
    ...
    # Adjust to hit exact n (distribute remainder to largest groups first)
    remainder = n - sum(allocation.values())
    if remainder > 0:
        # Sort by pool size descending to give extra samples to larger groups
        sorted_keys = sorted(groups.keys(), key=lambda k: len(groups[k]), reverse=True)
        for key in sorted_keys:
            if remainder <= 0:
                break
            # Only add if we haven't hit the pool size or max limit
    ...
    if remainder > 0:
        logger.warning(f"Could not allocate all {n} samples; short by {remainder}. "
                      f"Consider increasing min_per_group or reducing max_per_group.")
...
def compute_stratification_metrics(
    ...
    # Align keys and compute chi-square
    ...
    # Scale expected to test size
    scale = len(test_docs) / len(full_docs)
    expected_scaled = expected * scale
    
    # Chi-square test (handle zero expected values)
    ...
    # Compute max deviation (percentage points)
    deviations = []
    for key in all_keys:
    ...
    return {
        "chi2_statistic": round(float(chi2), 3) if chi2 is not None else None,
        "p_value": round(float(p_value), 4) if p_value is not None else None,
        "max_deviation_pct": round(max(deviations), 2),
        "mean_deviation_pct": round(np.mean(deviations), 2),
        "groups_matched": sum(1 for k in all_keys if full_counts.get(k, 0) > 0 and test_counts.get(k, 0) > 0),
...
def print_distribution_comparison(
    ...
    all_keys = sorted(set(full_counts) | set(test_counts))
    for key in all_keys[:20]:  # Show top 20 groups
        full_pct = full_counts.get(key, 0) / len(full_docs) * 100
        test_pct = test_counts.get(key, 0) / len(test_docs) * 100
...
def main() -> None:
    ...
    parser.add_argument("--stratify-by", type=str, nargs="+", default=DEFAULT_STRATIFY_BY,
                        help="Keys for stratification (e.g., course topic)")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    parser.add_argument("--min-per-group", type=int, default=3,
                        help="Minimum samples per group (default: 3)")
    parser.add_argument("--max-per-group", type=int, default=None,
                        help="Maximum samples per group (optional)")
    parser.add_argument("--exclude-outliers", action="store_true",
                        help="Exclude docs with topic==-1 from sampling pool")
    ...
    test_docs = stratified_sample_imbalanced(
        docs=docs,
        n=args.n,
    ...
    # Save metrics alongside for reference
