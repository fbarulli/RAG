...
from .core import (
    compute_recall_at_k,  # Alias for clarity
...
def aggregate_metrics(
    ...
    for r in results:
        ...
        if r.expected_id in r.hit_ids:
            ...
            ranks.append(11)  # Penalty for not found
            failure_count += 1
            if r.hit_scores:
...
def aggregate_metrics_by_topic(
    results: list[QueryResult],
    config_name: str,
    model_name: str,
) -> list[MetricSummary]:
    ...
    return [
        aggregate_metrics(group, config_name, model_name, topic=t, subtopic=st)
        for (t, st), group in sorted(groups.items())
    ]