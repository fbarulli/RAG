...
Run:
    uv run python -m production_pipeline.p04_ingestion.p02_ingest_models
    uv run python -m production_pipeline.p04_ingestion.p02_ingest_models --model BAAI/bge-base-en-v1.5
"""
...
def load_ner_map(topic_assignments_path: Optional[Path], model_name: str) -> dict[str, dict]:
    ...
    mapping = {
        ...
        for a in data["results"][model_name].get("assignments", [])
    }
...
def _prepare_payload(doc: FAQDocument, ner_info: dict) -> dict:
    ...
    return {
        ...
        "course": doc.course,
        "section": doc.section,
        "ner_category": ner_info["ner_category"],
        "ner_primary_entity": ner_info["ner_primary_entity"],
        "topic": ner_info["topic"],
        "subtopic": ner_info["subtopic"],
    }


def _create_points_batch(
    batch_docs: list[FAQDocument],
...
def _load_ner_map(topic_path: Path, model_name: str) -> dict[str, dict]:
    ...
    if model_name not in data["results"]:
        raise KeyError(
            f"Model '{model_name}' not found in topic assignments. "
            f"Run: uv run python -m production_pipeline.p02_eda.p02_topic_modeling "
            f"--embedding-model {model_name} --run-all"
        )
...
def ingest_one_model(
    *,
    ...
    # Get or compute embeddings (pass model_entry)
    ...
    # Load NER enrichment (hard fail if missing)
    ner_map = _load_ner_map(config.topic_path, model_name)
    
    # Verify all documents have NER data
    missing_docs = [doc.id for doc in docs if doc.id not in ner_map]
    if missing_docs:
        raise RuntimeError(
            f"❌ Model '{model_name}' missing NER data for {len(missing_docs)} documents.\n"
            f"   First 5 missing IDs: {missing_docs[:5]}\n\n"
            f"   Generate missing data with:\n"
            f"   uv run python -m production_pipeline.p02_eda.p02_topic_modeling \\\n"
            f"       --embedding-model {model_name} \\\n"
            f"       --run-all\n\n"
            f"   Then re-run this ingestion."
        )
...
def main() -> None:
    ...
    logger.info(
        f"Ingesting {len(model_entries)} model(s): "
        f"{[m['name'] for m in model_entries]}"
    )
