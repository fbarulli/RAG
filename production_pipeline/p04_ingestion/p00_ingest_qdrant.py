...
Run: uv run python -m production_pipeline.p04_ingestion.p00_ingest_qdrant --model "BAAI/bge-base-en-v1.5"
"""
import argparse
import json
import torch
        ...
        points = [
            PointStruct(
                ...
                payload={
                    ...
                    'course': doc.course,
                    'section': doc.section,
                    'ner_category': ner_map.get(doc.id, {}).get('ner_category', 'OTHER'),
                    'ner_primary_entity': ner_map.get(doc.id, {}).get('ner_primary_entity'),
                    'topic': ner_map.get(doc.id, {}).get('topic', -1),
                    'subtopic': ner_map.get(doc.id, {}).get('subtopic'),
                }
            )
            for j, (doc, vec) in enumerate(zip(batch_docs, batch_vecs), start=i)
        ]
