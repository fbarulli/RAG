...
class BenchmarkConfig:
    ...
    config: Optional[str] = None         # None → all retrieval configs
    ...
    batch_size: int = 100
    quick: bool = False                   # Limit queries for quick testing
    ...
    def from_defaults(cls) -> "BenchmarkConfig":
        ...
        # Helper to resolve relative paths against project root
        ...
        return cls(
            test_set_path=resolve(paths.get("test_jsonl")),
            clean_path=resolve(paths.get("clean_jsonl")),
            topic_path=resolve(paths.get("topic_assignments")),
            configs_path=resolve(paths.get("retrieval_configs")),
            output_dir=resolve(paths.get("experiments_dir")),
            cache_dir=resolve(paths.get("embeddings_cache_dir")),
            qdrant_host=qdrant.get("host", "localhost"),
            qdrant_port=qdrant.get("port", 6333),
            es_host=es.get("host"),
            es_index=es.get("index", "faqs"),
            top_k=bench.get("top_k", 10),
    ...
    def merge_args(self, args: argparse.Namespace) -> "BenchmarkConfig":
        ...
        """
        # Singular --model takes precedence over plural --models
        single_model: Optional[str] = getattr(args, "model", None)
        plural_models: Optional[List[str]] = getattr(args, "models", None)
        resolved_models = (
            [single_model] if single_model is not None
            else plural_models if plural_models is not None
            else self.models
        )
        ...
        return BenchmarkConfig(
            # paths
            test_set_path=_path("test_set_path", "test_set"),
            clean_path=_path("clean_path"),
            topic_path=_path("topic_path"),
            configs_path=_path("configs_path"),
            output_dir=_path("output_dir"),
            cache_dir=_path("cache_dir"),
            # qdrant
            qdrant_host=_val("qdrant_host"),
            qdrant_port=_val("qdrant_port"),
            # elasticsearch
            es_host=_val("es_host"),
            es_index=_val("es_index"),
            # models
            models=resolved_models,
            config=_val("config"),
            # benchmark settings
            top_k=_val("top_k"),
            encode_batch_size=_val("encode_batch_size"),
            batch_size=_val("batch_size"),
            quick=_bool_flag("quick"),
            # behaviour flags
            force_encode=_bool_flag("force_encode"),
            skip_existing=_bool_flag("skip_existing"),
            no_detail=_bool_flag("no_detail"),
            auto_prepare=_bool_flag("auto_prepare"),
            resume=_bool_flag("resume"),
            reset=_bool_flag("reset"),
            fail_fast=_bool_flag("fail_fast"),
            model_timeout=_val("model_timeout", "timeout"),
        )
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        # Qdrant will be checked on connection
        return len(issues) == 0, issues

    # ------------------------------------------------------------------
    # Lazy clients (cached properties)
    # ------------------------------------------------------------------
    ...
    def get_configs(self) -> dict:
        ...
        if self.configs_path is None:
            raise ValueError(
                "configs_path is not set — pass --configs-path or add "
                "'retrieval_configs' to defaults.json"
            )
        ...
        if self.config:
            if self.config not in configs:
                raise KeyError(
                    f"Config '{self.config}' not found. "
    ...
    def get_test_set(self, limit: Optional[int] = None) -> list[dict]:
        ...
        if self.test_set_path is None:
            raise ValueError(
                "test_set_path is not set — pass --test-set or add "
                "'test_jsonl' to defaults.json"
            )
