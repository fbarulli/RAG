...
# ---------------------------------------------------------------------------
# Config file locations (resolved relative to project root)
# ---------------------------------------------------------------------------
...
def _course_name_map() -> dict[str, str]:
    ...
    try:
        ...
        # Fallback for testing when defaults.json doesn't exist
...
def load_test_set(path: Path, clean_path: Optional[Path] = None) -> list[dict]:
    ...
    with path.open(encoding="utf-8") as f:
        for line_num, line in enumerate(f, 1):
            ...
            # Skip validation for OOD queries (expected_id is None or explicitly marked)
            is_ood = expected_id is None or doc.get("is_ood", False)
            
            if not is_ood and valid_ids and expected_id not in valid_ids:
                logger.warning(
                    f"Test set line {line_num}: expected_id '{expected_id}' not in corpus - skipping"
                )
                skipped += 1
                continue
            ...
            tests.append({
                ...
                "expected_id": expected_id,  # Will be None for OOD queries
                "course": course_map.get(doc["course"], doc["course"]) if doc.get("course") else None,
                "section": doc.get("section", ""),
                "answer": doc["answer"],
                "is_ood": is_ood,  # Add flag for downstream metrics
            })

    logger.info(f"Loaded {len(tests)} test queries from {path} ({skipped} skipped)")
    
    # Log how many OOD queries were loaded
...
def load_topic_assignments(path: Path, model: str) -> dict[str, dict]:
    ...
    if model not in data["results"]:
        ...
        raise KeyError(
            f"Model '{model}' not found in topic assignments. "
            f"Available: {available}"
        )
