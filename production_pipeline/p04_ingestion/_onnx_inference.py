...
class ONNXCrossEncoder:
    ...
    def predict(
        ...
        try:
            for batch_start in range(0, n_pairs, batch_size):
                ...
                # IMPROVEMENT: Safe, direct vectorized C-level assignment to pre-allocated array
    ...
    def _forward_batch(self, inputs: Dict[str, np.ndarray], batch_index: int) -> np.ndarray:
        ...
        try:
            # FIX: Optimum ORTModel wrappers require unpacking parameters (**kwargs).
            # This ensures input_ids, attention_mask, AND token_type_ids (if present) are handled.
            outputs = self.model(**inputs)
            
            # FIX: Extract the raw NumPy logits attribute out of the Optimum output dataclass
    ...
    def _extract_scores(self, logits: np.ndarray, batch_index: int) -> np.ndarray:
        ...
        try:
            ...
            if num_labels >= 2:
                # IMPROVEMENT: Stay in pure NumPy! Return array instead of list.
                # Protects against floating-point overflow warnings during np.exp()
