...
# Import the specialized ONNX wrapper
...
def _load_model_pytorch(model_name: str, max_length: int, use_quantization: bool) -> CrossEncoder:
    ...
    logger.info(
        "Loading reranker model: %r (max_length=%d, quantized=%s)",
        model_name,
        max_length,
        use_quantization,
    )
...
def _load_model_onnx(model_name: str, max_length: int) -> ONNXCrossEncoder:
    ...
    try:
        ...
        logger.warning(
            "ONNX load failed for %r: %s. Falling back to PyTorch fp32.\n%s",
            model_name,
            e,
...
def _get_model(
    ...
    """
    cache_key = (
        f"{model_name}:q{int(use_quantization)}:onnx{int(use_onnx)}:ml{max_length}"
    )
    ...
    with _CACHE_LOCK:
        if cache_key in _MODEL_CACHE:          # re-check under lock
            return _MODEL_CACHE[cache_key]
...
def _filter_valid_candidates(
    ...
    if n_filtered:
        logger.warning(
            "Reranker %r: %d candidate(s) missing 'es_id' filtered out before inference.",
            reranker_label,
            n_filtered,
        )
    if not clean:
        raise ValueError(
            f"Reranker '{reranker_label}': no valid candidates after filtering."
        )
...
# ---------------------------------------------------------------------------
# Helper functions for run_reranking
# ---------------------------------------------------------------------------
...
def _resolve_reranker_params(
    ...
    # Per-model quantization opt-out (e.g. ColBERT models set "quantization": false)
    ...
    logger.debug(
        "_resolve_reranker_params | label=%r model=%r max_length=%d "
        "use_quantization=%s use_onnx=%s batch_size=%d",
        reranker_label,
        model_name,
        max_length,
        use_quantization,
...
def _validate_candidates(
    ...
    if not candidates:
        logger.warning(
            "_validate_candidates | Reranker %r received an empty candidate list.",
            reranker_label,
        )
        return None
...
def _score_candidates(
    ...
    logger.debug(
        "_score_candidates | reranker=%r n_pairs=%d batch_size=%d",
        reranker_label,
        len(pairs),
        batch_size,
    )
    ...
    logger.debug(
        "_score_candidates | reranker=%r scores_shape=%s min=%.4f max=%.4f",
        reranker_label,
        scores.shape,
        float(scores.min()),
        float(scores.max()),
...
def _select_top_ids(
    ...
    logger.debug(
        "_select_top_ids | reranker=%r top_k=%d selected_ids=%s",
        reranker_label,
        top_k,
        reranked_ids,
    )
...
def _build_fallback(
    ...
    """
    logger.error(
        "_build_fallback | Reranker %r failed after %.1fms — "
        "returning original order fallback: %s\n%s",
        reranker_label,
        latency_ms,
        exc,
        traceback.format_exc(),
...
def run_reranking(
    ...
    """
    model_name, reranker_label, max_length, use_quantization, batch_size = (
        _resolve_reranker_params(reranker_config, use_quantization, use_onnx)
    )
    ...
    try:
        ...
        # --- score ---------------------------------------------------------
        scores_np = _score_candidates(
            model, query, candidates_clean, batch_size, reranker_label
        )
        ...
        logger.info(
            "run_reranking | Reranker %r done in %.1fms "
            "(%s, %d candidates → top %d)",
            reranker_label,
            latency_ms,
            mode,
