...
def main():
    ...
    for qid in samples:
        ...
        if raw_has_code and (not parsed_has_code):
            ...
            stats['ok'] += 1
        if status != 'OK':
            results.append({'id': qid, 'course': p_doc['course'], 'question': p_doc['question'], 'status': status, 'raw_body': r_body, 'parsed_answer': p_ans, 'raw_has_code': raw_has_code, 'parsed_has_code': parsed_has_code})
