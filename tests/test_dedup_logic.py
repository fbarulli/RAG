        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                if sim >= threshold:
                    ...
                    removals.append({'kept_id': group[keep]['id'], 'removed_id': group[remove]['id'], 'question': group[remove]['question'], 'kept_answer': group[keep]['answer'][:200], 'removed_answer': group[remove]['answer'][:200], 'similarity': round(sim, 4), 'course': group[remove]['course'], 'section': group[remove].get('section', '')})
    return removals
...
def main():
    ...
    parser.add_argument('--sample', type=int, default=DEFAULT_SAMPLE, help='Process only first N documents for quick testing')
    parser.add_argument('--output', type=Path, default=OUTPUT_PATH, help=f'Output JSON path (default: {OUTPUT_PATH})')
    ...
    report = {'input_count': len(docs), 'threshold': args.threshold, 'unique_questions': unique_questions, 'removal_count': len(removals), 'estimated_output': len(docs) - len(removals), 'removals': removals}
