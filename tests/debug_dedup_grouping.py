...
def inspect_grouping(docs: list[dict], filter_question: str=None):
    ...
    if filter_question:
        ...
        multi_groups = {k: v for k, v in multi_groups.items() if filter_question.lower() in k}
...
def search_expected_duplicates(docs: list[dict]):
    ...
    expected = [('capstone project evaluated', 'exact duplicate'), ('AUC to evaluate feature importance', 'near-duplicate'), ('evaluate feature importance', 'near-duplicate'), ('Prefect questions', 'bad answer')]
    print(f"\n{'#' * 70}")
...
def main():
    ...
    parser.add_argument('--question', type=str, default=None, help='Filter to show only groups containing this text')
    parser.add_argument('--search', action='store_true', help='Search for expected duplicates from audit')
