...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        ...
        help="Models to use (default: all enabled models in models.json)",
    )
    g.add_argument(
        "--config", type=str, default=None,
        help="Single retrieval config to run (default: all configs)",
    )
...
def _add_flag_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument("--force-encode",    action="store_true", default=False,
                   help="Re-encode even if a cached .npy file exists")
    g.add_argument("--skip-existing",   action="store_true", default=None,
                   help="Skip Qdrant collections that already have the right doc count")
    g.add_argument("--no-skip-existing", dest="skip_existing", action="store_false",
                   help="Always recreate Qdrant collections")
    g.add_argument("--no-detail",    action="store_true", default=False,
                   help="Skip per-config detail report")
    g.add_argument("--auto-prepare", action="store_true", default=False,
                   help="Auto-generate missing topic assignments before benchmarking")
    g.add_argument("--resume",  action="store_true", default=None,
                   help="Resume from a previous benchmark checkpoint")
    g.add_argument("--no-resume", dest="resume", action="store_false",
                   help="Start the benchmark from scratch")
    g.add_argument("--reset",     action="store_true", default=False,
...
def create_ingestion_parser() -> argparse.ArgumentParser:
    ...
    parser = create_base_parser(
        description="Ingest FAQ documents into Qdrant for one or all embedding models."
    )
    parser.add_argument(
