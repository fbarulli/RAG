...
Run: uv run python -m rag_pipeline.cleaning.p00_load_llm_queries
"""
import argparse
import json
...
def load_and_flatten(input_path: Path, output_path: Path) -> int:
    ...
    for q in data['queries']:
        ...
        queries.append({'id': f'{base_id}_orig', 'question': original_question, 'expected_doc_id': base_id, 'course': course, 'answer': answer, 'query_type': 'original', 'source': 'llm_generated'})
        for qtype, variations in q.get('prompt_results', {}).items():
            for i, var in enumerate(variations):
                queries.append({'id': f'{base_id}_{qtype}_{i}', 'question': var, 'expected_doc_id': base_id, 'course': course, 'answer': answer, 'query_type': qtype, 'source': 'llm_generated'})
