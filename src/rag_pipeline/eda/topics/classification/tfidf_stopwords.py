...
BRIDGE_CONCEPT_WHITELIST: frozenset[str] = frozenset({'docker', 'python', 'gcp', 'dbt', 'aws', 'bigquery', 'mlflow', 'spark', 'kestra', 'pandas', 'jupyter', 'api', 'github', 'pipenv', 'airflow', 'kafka', 'kubernetes', 'terraform', 'postgres', 'sql', 'flask', 'fastapi', 'prefect', 'mage', 'dagster', 'langchain', 'openai', 'huggingface', 'bert', 'llm', 'rag', 'sklearn', 'xgboost', 'lightgbm', 'deploy', 'deployment', 'monitor', 'monitoring', 'production', 'run', 'pipeline', 'train', 'training', 'inference', 'serve', 'debug', 'error', 'install', 'configure', 'setup'})

def frequency_based_stopwords(df: pd.DataFrame, text_col: str='clean_text', presence_threshold: float=0.85, min_chunk_freq: int=5) -> set[str]:
    """
    ...
    try:
        ...
        vec = CountVectorizer(ngram_range=(1, 1), min_df=min_chunk_freq, stop_words='english')
        matrix = vec.fit_transform(df[text_col])
        feature_names = vec.get_feature_names_out()
        doc_freq = np.asarray((matrix > 0).sum(axis=0)).flatten()
        presence = doc_freq / len(df)
        stopwords = {feature_names[i] for i, p in enumerate(presence) if p >= presence_threshold and feature_names[i] not in BRIDGE_CONCEPT_WHITELIST}
        logger.info(f'[frequency_based_stopwords] threshold={presence_threshold}, min_df={min_chunk_freq} -> {len(stopwords)} stopwords from {len(df)} documents')
        logger.debug(f'Sample: {sorted(stopwords)[:20]}')
        return stopwords
    except Exception:
        logger.error('[frequency_based_stopwords] Failed.\n' + traceback.format_exc())
        raise

def saturation_based_stopwords(tfidf_results: dict[str, list[tuple[str, float]]], top_n: int=50, presence_threshold: float=0.6) -> set[str]:
    """
    ...
    try:
        ...
        if n_courses < 2:
            logger.warning('[saturation_based_stopwords] Only one course found — returning empty set.')
            return set()
        term_count: dict[str, int] = defaultdict(int)
        for course, terms in tfidf_results.items():
            if not terms:
                logger.warning(f"[saturation_based_stopwords] Course '{course}' has an empty term list — skipping.")
                continue
            for term, _ in terms[:top_n]:
                term_count[term] += 1
        cutoff = max(2, int(n_courses * presence_threshold))
        stopwords = {term for term, count in term_count.items() if count >= cutoff and term not in BRIDGE_CONCEPT_WHITELIST}
        logger.info(f'[saturation_based_stopwords] top_n={top_n}, threshold={presence_threshold} (cutoff={cutoff}/{n_courses} courses) -> {len(stopwords)} stopwords')
        logger.debug(f'Sample: {sorted(stopwords)[:20]}')
        return stopwords
    except Exception:
        logger.error('[saturation_based_stopwords] Failed.\n' + traceback.format_exc())
        raise

def idf_floor_stopwords(vectorizer: TfidfVectorizer, percentile: float=3.0) -> set[str]:
    """
    ...
    try:
        ...
        stopwords = {feature_names[i] for i, score in enumerate(idf_scores) if score <= threshold and feature_names[i] not in BRIDGE_CONCEPT_WHITELIST}
        logger.info(f'[idf_floor_stopwords] percentile={percentile}, idf_threshold={threshold:.4f} -> {len(stopwords)} stopwords')
        logger.debug(f'Sample: {sorted(stopwords)[:20]}')
        return stopwords
    except Exception:
        logger.error('[idf_floor_stopwords] Failed.\n' + traceback.format_exc())
        raise

def eda_derived_stopwords(eda_stats: dict, top_words_keep_pct: float=0.4) -> set[str]:
    """
    ...
    try:
        if 'top_words' not in eda_stats:
            ...
            raise KeyError(f"'top_words' key not found in eda_stats. Available keys: {list(eda_stats.keys())}")
        if not 0.0 <= top_words_keep_pct <= 1.0:
            raise ValueError(f'top_words_keep_pct must be in [0, 1], got {top_words_keep_pct}')
        ...
        frequency_stops = {word for word, _ in sorted_words[n_keep:] if word not in BRIDGE_CONCEPT_WHITELIST}
        structural_noise = {'homework', 'course', 'module', 'project', 'running', 'run', 'find', 'set', 'start', 'create', 'code', 'after', 'was', 'your', 'failed', 'error'} - BRIDGE_CONCEPT_WHITELIST
        stopwords = frequency_stops | structural_noise
        logger.info(f'[eda_derived_stopwords] top_words_keep_pct={top_words_keep_pct} -> {len(frequency_stops)} frequency stops + {len(structural_noise)} structural-noise terms = {len(stopwords)} total')
        logger.debug(f'Sample: {sorted(stopwords)[:20]}')
        return stopwords
    except Exception:
        logger.error('[eda_derived_stopwords] Failed.\n' + traceback.format_exc())
        raise

def balance_corpus(df: pd.DataFrame, course_col: str='course', strategy: str='oversample_min', random_state: int=42) -> pd.DataFrame:
    """
    ...
    try:
        if course_col not in df.columns:
            raise ValueError(f"Column '{course_col}' not found. Available columns: {list(df.columns)}")
        counts = df[course_col].value_counts()
        median = int(counts.median())
        logger.info(f'[balance_corpus] strategy={strategy}, course counts before: {counts.to_dict()}, target={median}')
        if strategy == 'none':
            return df.copy()
        ...
        for course, group in df.groupby(course_col):
            ...
            if strategy == 'oversample_min':
                ...
                raise ValueError(f"Unknown strategy '{strategy}'. Choose from: oversample_min, undersample_max, none")
        result = pd.concat(frames).reset_index(drop=True)
        logger.info(f'[balance_corpus] course counts after: {result[course_col].value_counts().to_dict()}')
        return result
    except Exception:
...
def apply_code_stripping(df: pd.DataFrame, text_col: str='clean_text', output_col: str='tfidf_text') -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(f'[apply_code_stripping] {text_col} -> {output_col}: {n_changed}/{len(df)} chunks modified')
        return df
    except Exception:
        logger.error('[apply_code_stripping] Failed.\n' + traceback.format_exc())
        raise

def build_stopword_list(df: pd.DataFrame | None=None, eda_stats: dict | None=None, tfidf_results: dict | None=None, vectorizer: TfidfVectorizer | None=None, extra: set[str] | None=None) -> list[str]:
    """
    ...
    try:
        ...
        if extra:
            ...
            if protected:
                logger.warning(f'[build_stopword_list] Ignoring {len(protected)} whitelisted terms passed in extra: {sorted(protected)}')
        ...
        logger.error('[build_stopword_list] Failed.\n' + traceback.format_exc())
        raise

def save_stopwords(stopwords: list[str], output_dir: Path, filename: str='stopwords.txt') -> Path:
    """
