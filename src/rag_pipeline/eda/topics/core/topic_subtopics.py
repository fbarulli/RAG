...
def build_subtopics(assignments: list, questions: list[str], embeddings: np.ndarray, subtopic_threshold: int, subtopic_min_size: int) -> dict[str, SubtopicRecord]:
    ...
    for i, parent_topic in enumerate(sorted(large_topics), 1):
        ...
        sub_model = BERTopic(embedding_model=_PRECOMPUTED, min_topic_size=subtopic_min_size, verbose=False)
        try:
            sub_topics, _ = sub_model.fit_transform(parent_questions, parent_embeddings)
        except Exception as exc:
            logger.warning(f'Subtopic clustering failed for topic {parent_topic} ({len(parent_indices)} docs): {exc} — skipping')
            continue
        for local_idx, global_idx in enumerate(parent_indices):
            ...
            keywords: list[str] = [word for word, _ in raw_topic[:_KEYWORDS_PER_SUBTOPIC]] if raw_topic is not False else []
            subtopic_cache[doc_id] = SubtopicRecord(subtopic=sub_t, subtopic_keywords=keywords)
