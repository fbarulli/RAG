...
def load_assignments(path: Path) -> list[dict]:
    ...
    if not assignments:
        raise ValidationError("No assignments found — check input file structure (expected key: 'assignments')")
    return assignments
...
def _compute_keyword_overlap(topic_keywords: dict[int, set[str]]) -> float | None:
    ...
    logger.info(f'Topic count {n} exceeds exact limit ({_OVERLAP_EXACT_LIMIT}); estimating keyword overlap from {_OVERLAP_SAMPLE_PAIRS} random pairs.')
    total = 0.0
...
def compute_raw_metrics(assignments: list[dict]) -> dict[str, Any]:
    ...
    size_stats: dict[str, float] = {'min': float(min(sizes)) if sizes else 0.0, 'max': float(max(sizes)) if sizes else 0.0, 'mean': round(mean(sizes), 1) if sizes else 0.0, 'median': round(median(sizes), 1) if sizes else 0.0, 'std': round(stdev(sizes), 1) if len(sizes) > 1 else 0.0}
    ...
    probs = [a['topic_probability'] for a in assignments if a.get('topic', -1) != -1 and 'topic_probability' in a]
    confidence_stats: dict[str, Any] | None = None
    if probs:
        low_conf = sum((1 for p in probs if p < 0.5))
        confidence_stats = {'mean': round(mean(probs), 4), 'low_confidence_count': low_conf, 'low_confidence_ratio': round(low_conf / len(probs), 4)}
    ...
    return {'total_documents': total, 'num_topics': len(real_topics), 'outlier_count': outliers, 'outlier_ratio': round(outlier_ratio, 4), 'topic_sizes': {str(k): v for k, v in sorted(real_topics.items())}, 'topic_size_stats': size_stats, 'largest_topic_share': round(largest_topic_share, 4), 'tiny_topic_count': tiny_topic_count, 'tiny_topic_ids': tiny_topic_ids, 'confidence_stats': confidence_stats, 'cross_course_topics': cross_course_count, 'avg_keyword_overlap': keyword_overlap}

def evaluate_status(metrics: dict[str, Any], outlier_thresh: float, tiny_thresh: int, conc_thresh: float, confidence_fail_thresh: float=0.4) -> tuple[str, list[str]]:
    """
    ...
    if metrics['outlier_ratio'] > outlier_thresh:
        violations.append(f"outlier_ratio {metrics['outlier_ratio']:.1%} > threshold {outlier_thresh:.1%}")
    if metrics['tiny_topic_count'] > tiny_thresh:
        violations.append(f"tiny_topics {metrics['tiny_topic_count']} > threshold {tiny_thresh}")
    if metrics['largest_topic_share'] > conc_thresh:
        violations.append(f"concentration {metrics['largest_topic_share']:.1%} > threshold {conc_thresh:.1%}")
    if metrics.get('confidence_stats') and metrics['confidence_stats']['mean'] < confidence_fail_thresh:
        violations.append(f"mean_confidence {metrics['confidence_stats']['mean']:.3f} < {confidence_fail_thresh}")
    if not violations:
        return ('OK', [])
    if len(violations) == 1:
...
def main() -> None:
    ...
    parser.add_argument('--outlier-threshold', type=float, default=0.15, help='Max acceptable outlier ratio (default: 0.15)')
    parser.add_argument('--tiny-topic-threshold', type=int, default=3, help='Max acceptable number of tiny topics ≤3 docs (default: 3)')
    parser.add_argument('--concentration-threshold', type=float, default=0.3, help='Max acceptable largest-topic share of total documents (default: 0.30)')
    parser.add_argument('--confidence-fail-threshold', type=float, default=0.4, help='Mean confidence below this value triggers a violation (default: 0.4)')
    ...
    status, violations = evaluate_status(metrics, args.outlier_threshold, args.tiny_topic_threshold, args.concentration_threshold, args.confidence_fail_threshold)
    metrics['status'] = status
    metrics['violations'] = violations
    metrics['thresholds_used'] = {'outlier': args.outlier_threshold, 'tiny_topic': args.tiny_topic_threshold, 'concentration': args.concentration_threshold, 'confidence_fail': args.confidence_fail_threshold}
