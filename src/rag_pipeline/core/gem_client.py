...
Usage tracking:
  ...
  - Read the log any time:
        from rag_pipeline.core.gem_client import read_usage_log
        print(read_usage_log())

Quick-start:
  Sync call:
        from rag_pipeline.core.gem_client import call_gemini
        result = call_gemini("Say hi!", max_tokens=50)
        print(result.content)

  Async call (with shared rate limiter for concurrent workloads):
        import asyncio
        from rag_pipeline.core.gem_client import call_gemini_async, GeminiRateLimiter
...
TPM_LIMIT = 250000
RPD_LIMIT = 250
RPM_SAFE = RPM_LIMIT - 1
TPM_SAFE = int(TPM_LIMIT * 0.96)
RPD_SAFE = RPD_LIMIT - 10
...
def init():
    ...
    if env_path:
        ...
        logger.warning('No .env file found in any parent directory; GEMINI_API_KEY must already be set in the environment.')
...
def _record_usage_json(result: 'GeminiResult') -> None:
    ...
    if today not in data['daily']:
        data['daily'][today] = {'requests': 0, 'prompt_tokens': 0, 'completion_tokens': 0, 'total_tokens': 0}
    ...
    entry = {'timestamp_pt': datetime.now(PACIFIC).isoformat(), 'model': result.model, 'prompt_tokens': result.prompt_tokens, 'completion_tokens': result.completion_tokens, 'total_tokens': result.total_tokens, 'latency_ms': round(result.latency_ms, 1)}
...
class GeminiRateLimiter:
    ...
    async def wait_if_needed(self, estimated_tokens: int=0):
        ...
        async with self._lock:
            ...
            if current_rpm >= RPM_SAFE or current_tpm + estimated_tokens > TPM_SAFE or self._daily_requests >= RPD_SAFE:
                if self._requests:
                    oldest_ts = self._requests[0][0]
                    wait_time = max(0.1, oldest_ts + 60 - time.monotonic())
                else:
                    wait_time = 1.0
                logger.warning(f'GeminiRateLimiter: approaching limits (RPM:{current_rpm}/{RPM_LIMIT}, TPM:~{current_tpm}/{TPM_LIMIT}, RPD:{self._daily_requests}/{RPD_LIMIT}). Waiting {wait_time:.1f}s...')
...
def _is_rate_limit_error(resp_or_exc) -> bool:
    if isinstance(resp_or_exc, Exception):
        ...
        msg = str(getattr(resp_or_exc, 'text', '')) + str(getattr(resp_or_exc, 'status_code', ''))
...
def _build_payload(prompt: str, max_tokens: int, temperature: float=0.3) -> dict:
    return {'contents': [{'parts': [{'text': prompt}]}], 'generationConfig': {'maxOutputTokens': max_tokens, 'temperature': temperature}}

def call_gemini(prompt: str, max_tokens: int, model: str=DEFAULT_MODEL, temperature: float=0.3) -> GeminiResult:
    """
    ...
    if not api_key:
        raise ValueError('GEMINI_API_KEY not found. Add it to your .env file as:\n    GEMINI_API_KEY=AIza...')
    estimated_prompt_tokens = _count_tokens(prompt)
    url = f'https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent'
    for attempt in range(MAX_RETRIES):
        try:
            t0 = time.monotonic()
            resp = requests.post(url, headers={'Content-Type': 'application/json'}, params={'key': api_key}, json=_build_payload(prompt, max_tokens, temperature), timeout=120)
            elapsed = (time.monotonic() - t0) * 1000
            if resp.status_code != 200:
                if _is_rate_limit_error(resp):
                    wait = RATE_LIMIT_WAIT * (attempt + 1)
                    logger.warning(f'Rate limit hit (HTTP {resp.status_code}), waiting {wait}s')
                    time.sleep(wait)
                    continue
            ...
            result = GeminiResult(content=content.strip(), latency_ms=elapsed, model=model, prompt_tokens=estimated_prompt_tokens, completion_tokens=completion_tokens, total_tokens=estimated_prompt_tokens + completion_tokens)
            ...
            logger.warning(f'Gemini error (attempt {attempt + 1}): {e} — waiting {wait:.0f}s')
            time.sleep(wait)

async def call_gemini_async(prompt: str, max_tokens: int, model: str=DEFAULT_MODEL, temperature: float=0.3, rate_limiter: Optional[GeminiRateLimiter]=None) -> GeminiResult:
    """
        ...
        try:
            ...
            async with asyncio.timeout(120):
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, headers={'Content-Type': 'application/json'}, params={'key': api_key}, json=_build_payload(prompt, max_tokens, temperature)) as resp:
                        if resp.status != 200:
            ...
            logger.warning(f'Gemini async error (attempt {attempt + 1}): {e} — sleeping {wait:.1f}s')
