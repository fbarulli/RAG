...
def call_llm(prompt: str, max_tokens: int, model: str=DEFAULT_MODEL, temperature: float=0.3, system: str=None) -> LLMResult:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            return LLMResult(content=content.strip(), latency_ms=elapsed, model=model, prompt_tokens=usage.prompt_tokens if usage else 0, completion_tokens=usage.completion_tokens if usage else 0)
            ...
            if attempt == MAX_RETRIES - 1:
                ...
                raise
            if is_rl:
                logger.warning(f'Rate limit hit (attempt {attempt + 1}/{MAX_RETRIES}) — waiting {wait:.0f}s before retry...')
                for remaining in range(int(wait), 0, -10):
                    logger.info(f'  Resume in {remaining}s...')
                    time.sleep(min(10, remaining))
            else:
                logger.warning(f'LLM error (attempt {attempt + 1}/{MAX_RETRIES}): {e} — retrying in {wait:.0f}s')
                time.sleep(wait)

async def call_llm_async(prompt: str, max_tokens: int, model: str=DEFAULT_MODEL, temperature: float=0.3, rate_gate: RateGate=None, system: str=None) -> LLMResult:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            if system:
                messages.append({'role': 'system', 'content': system})
            messages.append({'role': 'user', 'content': prompt})
            resp = await acompletion(model=model, messages=messages, temperature=temperature, max_tokens=max_tokens)
            elapsed = (time.monotonic() - t0) * 1000
            content = resp.choices[0].message.content
            if content is None:
                content = ''
