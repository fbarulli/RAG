...
Run:
    ...
    uv run python -m rag_pipeline.evaluation.llm_judge --model "BAAI/bge-base-en-v1.5" --config "hybrid_rrf"
"""
import argparse
import json
import traceback
...
def build_answer_map(collection: str, client: QdrantClient) -> dict[str, str]:
    """Fetch all es_id -> answer mappings from Qdrant collection."""
    answer_map = {}
    offset = None
    while True:
        result = client.scroll(collection_name=collection, limit=100, offset=offset, with_payload=True, with_vectors=False)
...
def score_result(query: str, reference: str, retrieved: str | None) -> dict[str, float]:
    """Call LLM judge and parse scores."""
    if not retrieved:
        return {'faithfulness': 0.0, 'factual_correctness': 0.0, 'latency_ms': 0.0}
    prompt = JUDGE_PROMPT.format(query=query, reference=reference, retrieved=retrieved)
    try:
        result: LLMResult = call_llm(prompt=prompt, max_tokens=150, temperature=0.0)
        raw = result.content.strip()
        logger.info(f'[score_result] tokens=prompt:{result.prompt_tokens} completion:{result.completion_tokens} latency={result.latency_ms:.0f}ms')
        ...
        return {'faithfulness': float(scores.get('faithfulness', 0.0)), 'factual_correctness': float(scores.get('factual_correctness', 0.0)), 'latency_ms': result.latency_ms}
    except Exception:
        logger.warning(f'[score_result] Failed to parse judge response:\n{traceback.format_exc()}')
        return {'faithfulness': 0.0, 'factual_correctness': 0.0, 'latency_ms': 0.0}

def main(model_name: str=DEFAULT_MODEL, config_name: str=DEFAULT_CONFIG, query_results_path: Path=DEFAULT_QUERY_RESULTS, output_path: Path=DEFAULT_OUTPUT, limit: int | None=None) -> None:
    logger.info(f'[llm_judge] model={model_name} config={config_name}')
    ...
    target_results = [r for r in all_results if r.get('model') == model_name and r.get('config') == config_name]
    if limit:
        target_results = target_results[:limit]
        logger.info(f'[llm_judge] Limited to {limit} queries for test run')
    logger.info(f'[llm_judge] {len(target_results)} queries to judge')
    if not target_results:
        raise ValueError(f"No results found for model={model_name} config={config_name}. Available: {list(set(((r['model'], r['config']) for r in all_results)))}")
    ...
    if output_path.exists():
        try:
            ...
            logger.info(f'[llm_judge] Resuming — {len(existing)} already scored')
        except Exception:
            logger.warning('[llm_judge] Could not load existing results — starting fresh')
    ...
    for i, r in enumerate(target_results):
        ...
        scores = score_result(query=query_text, reference=ref_answer, retrieved=retrieved_answer)
        judge_result = {'query_id': query_id, 'query_text': query_text, 'expected_id': expected_id, 'top_hit_id': top_hit_id, 'hit': r.get('hit', False), 'query_type': r.get('query_type', 'unknown'), 'course': r.get('course', ''), 'model': model_name, 'config': config_name, **scores}
        judge_results.append(judge_result)
        scored += 1
        logger.info(f"[{i + 1}/{len(target_results)}] {query_id} | hit={r.get('hit')} | faith={scores['faithfulness']:.2f} fc={scores['factual_correctness']:.2f} | latency={scores.get('latency_ms', 0):.0f}ms | {query_text[:60]}")
        if scored % BATCH_SAVE_EVERY == 0:
            _save(judge_results, output_path)
            logger.info(f'[llm_judge] Checkpoint saved ({scored} scored)')
    _save(judge_results, output_path)
    _print_summary(judge_results, model_name, config_name)
    logger.info(f'[llm_judge] Done — {scored} scored, {skipped} skipped (resume)')
...
def _print_summary(results: list[dict], model: str, config: str) -> None:
    ...
    composite_non_chaos = sum(((r['faithfulness'] + r['factual_correctness']) / 2 for r in non_chaos)) / len(non_chaos) if non_chaos else 0
    composite_chaos = sum((r['factual_correctness'] for r in chaos)) / len(chaos) if chaos else 0
    print(f"\n{'=' * 70}")
    ...
    for qt, items in sorted(by_type.items()):
        ...
        print(f'    {qt:20s} n={len(items):3d} faith={sum(f_scores) / len(f_scores):.3f} fc={sum(fc_scores) / len(fc_scores):.3f} [{primary}]')
    print(f"{'=' * 70}\n")
if __name__ == '__main__':
    ...
    main(model_name=args.model, config_name=args.config, query_results_path=args.query_results, output_path=args.output, limit=args.limit)