...
def main():
    ...
    try:
        ...
        for cfg_name, cfg in configs.items():
            ...
            results = evaluate_config(client=config.qdrant_client, collection=model_entry['collection'], model=model, test_set=test_set, topic_map=topic_map, config=cfg, top_k=config.top_k, es=config.es_client, es_index=config.es_index, encode_batch_size=config.encode_batch_size)
