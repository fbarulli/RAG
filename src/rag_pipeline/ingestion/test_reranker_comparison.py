...
def recall_at_k(results: List[Dict], reranker_name: str, k: int) -> float:
    hits = sum((1 for r in results if r.get('expected_id') in r.get(f'{reranker_name}_top_ids', [])[:k]))
...
def _load_test_queries(config: BenchmarkConfig, sample_size: int) -> List[Dict]:
    try:
        from rag_pipeline.cleaning.p04_stratified_test_split import create_stratified_sample
...
def _build_embedder(expected_dim: int=768):
    ...
    if actual_dim != expected_dim:
        logger.error(f"Dimension mismatch: '{PRODUCTION_MODEL}' produces {actual_dim}-dim vectors but '{PRODUCTION_COLLECTION}' expects {expected_dim}-dim. Aborting.")
        sys.exit(1)
...
def _retrieve_candidates(embedder, qdrant_client, query: str, item: Dict, top_k: int) -> tuple:
    ...
    initial = run_entity_boosted_retrieval(ner_category=item.get('ner_category'), ner_primary_entity=item.get('ner_primary_entity'), client=qdrant_client, collection=PRODUCTION_COLLECTION, query_vector=query_vector, course_filter='', config={}, top_k=top_k * RETRIEVAL_CANDIDATE_MULTIPLIER)
    ...
    candidates = [{'es_id': doc_id, 'payload': {'es_id': doc_id, 'question': query, 'answer': hit_answers[j]}} for j, doc_id in enumerate(initial.hit_ids)]
    return (candidates, retrieval_latency_ms)

def _run_rerankers(query: str, item: Dict, candidates: List[Dict], reranker_names: List[str], top_k: int, retrieval_latency_ms: float) -> Dict:
    """Run all rerankers on the same candidate set and return a result dict."""
    result: Dict = {'query': query, 'topic': item.get('course', 'unknown'), 'expected_id': item.get('expected_id'), 'retrieval_latency_ms': retrieval_latency_ms}
    for reranker_name in reranker_names:
        ids, metrics = evaluate_with_reranker(query=query, retrieved_candidates=candidates, reranker_name=reranker_name, top_k=top_k)
        result[f'{reranker_name}_top_ids'] = ids
        result[f'{reranker_name}_latency_ms'] = metrics.get('reranker_latency_ms', 0.0)
    return result

def _process_queries(test_queries: List[Dict], processed: set, embedder, qdrant_client, reranker_names: List[str], top_k: int, ndj_path: Path) -> List[Dict]:
    ...
    with open(ndj_path, 'a') as ndj_file:
        for i, item in enumerate(test_queries):
            ...
            try:
                candidates, retrieval_latency_ms = _retrieve_candidates(embedder, qdrant_client, query, item, top_k)
                result = _run_rerankers(query, item, candidates, reranker_names, top_k, retrieval_latency_ms)
...
def main() -> None:
    ...
    if args.dry_run:
        logger.info(f"Dry-run OK — {len(test_queries)} queries, collection: '{PRODUCTION_COLLECTION}', rerankers: {args.rerankers}")
        sys.exit(0)
    ...
    results = _process_queries(test_queries=test_queries, processed=processed, embedder=embedder, qdrant_client=qdrant_client, reranker_names=args.rerankers, top_k=args.top_k, ndj_path=ndj_path)
    if not results:
