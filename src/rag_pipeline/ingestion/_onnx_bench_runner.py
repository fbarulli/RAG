...
def _run_matrix_evaluation(client: QdrantClient, test_set: List[Any], embedding_entry: Dict[str, Any], reranker_entries: List[Dict[str, Any]], retrieval_config: Dict[str, Any], embedding_model: SentenceTransformer, topic_map: Dict[str, Any] | None, target_override: str | None) -> List[Any]:
    """RESPONSIBILITY: Delegates to the core matrix evaluation loop and returns summaries."""
    return execute_matrix_evaluation(client=client, test_set=test_set, embedding_entry=embedding_entry, reranker_entries=reranker_entries, retrieval_config=retrieval_config, embedding_model=embedding_model, topic_map=topic_map, target_override=target_override)
...
def main() -> None:
    ...
    try:
        ...
        summaries = _run_matrix_evaluation(client=client, test_set=test_set, embedding_entry=embedding_entry, reranker_entries=reranker_entries, retrieval_config=retrieval_config, embedding_model=embedding_model, topic_map=topic_map, target_override=getattr(args, 'reranker', None))
