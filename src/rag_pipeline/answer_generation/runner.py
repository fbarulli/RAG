...
rag_pipeline/answer_generation/runner.py
    ...
    with open(test_set_path) as f:
        for line in f:
            if not line.strip():
                continue
            doc = json.loads(line)
            queries.append({'query_id': doc['id'], 'query_text': doc['question'], 'expected_id': doc.get('expected_id') or doc.get('expected_doc_id') or doc['id'], 'answer': doc.get('answer', '')})
    if limit:
...
class QueryRetriever:
    ...
    def retrieve(self, query: str, top_k: int=5, course: Optional[str]=None, mode: str='vector') -> list[str]:
        ...
        if mode == 'entity_boosted':
            ...
            result = run_hybrid_rrf_retrieval(client=self.client, collection=self.collection, query_vector=vector, es=None, es_index='faqs', query_text=query, course_filter=course, config=config, top_k=top_k)
        else:
            result = run_vector_retrieval(client=self.client, collection=self.collection, query_vector=vector, course_filter=course, config=config, top_k=top_k)
        return list(result.hit_ids)
...
def generate_single(query_id: str, query: str, expected_id: str, reference_answer: str, retrieved_ids: list[str], prompt_style: str, top_k: int, generator: AnswerGenerator, evaluator: AnswerEvaluator, context_retriever: ContextRetriever, rerank: bool=True) -> PipelineResult:
    ...
    generation = generator.generate(query_id=query_id, query=query, context_doc_ids=final_ids, context_text=context_text, prompt_style=prompt_style)
    if generation.success:
        evaluation = evaluator.evaluate(query_id=query_id, query=query, generated_answer=generation.generated_answer, reference_answer=reference_answer)
    else:
        evaluation = EvaluationResult(query_id=query_id, faithfulness=0.0, factual_correctness=0.0, latency_ms=0, prompt_tokens=0, completion_tokens=0, error=generation.error)
    return PipelineResult(query_id=query_id, query=query, expected_id=expected_id, retrieved_doc_ids=final_ids, generation=asdict(generation), evaluation=asdict(evaluation), prompt_style=prompt_style, top_k=top_k, timestamp=datetime.now().isoformat(), reranked=rerank)
    ...
    for query_data in tqdm(test_queries, desc='Generating answers'):
        ...
        for style in prompt_styles:
            for top_k in top_k_values:
                ...
                result = generate_single(query_id=query_id, query=query, expected_id=expected_id, reference_answer=reference_answer, retrieved_ids=candidate_ids, prompt_style=style, top_k=top_k, generator=generator, evaluator=evaluator, context_retriever=context_retriever, rerank=rerank)
...
def print_summary(results: list[PipelineResult], styles: list[str], top_k_values: list[int]) -> None:
    """Print summary table broken down by prompt_style × top_k."""
    col_w = 10
    print('\n' + '=' * 110)
    print('ANSWER GENERATION SUMMARY')
    print('=' * 110)
    print(f"{'Style':<12} {'top_k':>5}  {'Faithful':>{col_w}} {'Factual':>{col_w}} {'Gen p50ms':>{col_w}} {'Judge p50ms':>{col_w}} {'Prompt tok':>{col_w}} {'Comp tok':>{col_w}} {'Total tok':>{col_w}}  {'N':>5}")
    print('-' * 110)
    best_score = -1.0
    best_label = ''
    for style in styles:
        for top_k in top_k_values:
            ...
            total_tok = _avg([r.generation['prompt_tokens'] + r.generation['completion_tokens'] for r in bucket])
            combined = (faithful + factual) / 2
            label = f'{style}/top_k={top_k}'
            if combined > best_score:
                best_score = combined
                best_label = label
            print(f'{style:<12} {top_k:>5}  {faithful:>{col_w}.3f} {factual:>{col_w}.3f} {gen_lat:>{col_w}.0f} {judge_lat:>{col_w}.0f} {prompt_tok:>{col_w}.0f} {comp_tok:>{col_w}.0f} {total_tok:>{col_w}.0f}  {len(bucket):>5}')
        print()
    print('=' * 110)
...
def run_pipeline(retrieval_model: Optional[str]=None, llm_model: Optional[str]=None, prompt_styles: Optional[list[str]]=None, top_k_values: Optional[list[int]]=None, limit: Optional[int]=None, rerank: bool=True) -> None:
    ...
    with open(defaults_path) as f:
        defaults = json.load(f)
    qdrant_cfg = defaults.get('qdrant', {})
    context_retriever = ContextRetriever(host=qdrant_cfg.get('host', 'localhost'), port=qdrant_cfg.get('port', 6333), model_name=retrieval_model)
    ...
    results = run_generations(test_queries=test_queries, retriever=retriever, context_retriever=context_retriever, generator=generator, evaluator=evaluator, prompt_styles=prompt_styles, top_k_values=top_k_values, rerank=rerank)
...
def main():
    ...
    run_pipeline(retrieval_model=args.model or None, llm_model=args.llm_model or None, prompt_styles=args.styles, top_k_values=args.top_k_list, limit=args.limit, rerank=getattr(args, 'rerank', True))
