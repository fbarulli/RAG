...
def run_ingestion() -> None:
    ...
    for model in MODELS:
        ...
        run([sys.executable, '-m', 'rag_pipeline.ingestion.p00_ingest_qdrant', '--model', model], step='ingest_qdrant')
    logger.info('  ES: BAAI/bge-base-en-v1.5 (model-agnostic BM25)')
    run([sys.executable, '-m', 'rag_pipeline.ingestion.p00_ingest_es', '--model', 'BAAI/bge-base-en-v1.5'], step='ingest_es')
...
def run_benchmark() -> None:
    ...
    for model in MODELS:
        ...
        run([sys.executable, '-m', 'rag_pipeline.ingestion.p03_benchmark', '--model', model, '--test-set', str(TEST_FILE)], step='benchmark')
...
def print_summary() -> None:
    ...
    for r in sorted(results, key=lambda x: -x.get('hit_rate_5', 0)):
        if r.get('num_queries', 0) == 0:
            continue
        print(f"  {r.get('model_name', '?'):<43}   {r.get('config_name', '?'):<18}   {r.get('hit_rate_5', 0):>5.1%}   {r.get('mrr', 0):>5.3f}   {r.get('latency_p50', 0):>6.1f}ms")
    print(f"{'=' * 90}")
    ...
    if judge_path.exists():
        ...
        composite = sum(((r['faithfulness'] + r['factual_correctness']) / 2 for r in non_chaos)) / len(non_chaos) if non_chaos else 0
